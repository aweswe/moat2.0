...
class CandidateGenerator:
    ...
    def generate(
        ...
        legacy_handlers = {
            ...
            "runtime_error": self._generate_generic_wrap,
            "unknown_error": self._generate_generic_wrap,
        }
...
{patch_code}""",
    ...
    def _generate_generic_wrap(self, rca: RCAResult, detection_id: str) -> List[AFECandidate]:
        """Generate a generic try-except wrap for unhandled error types."""
        vars = rca.variables
        
        # Try to get the failing line from context
        original_line = vars.get("original_line") or vars.get("failing_line", "# <failing code>")
        exception_type = vars.get("exception_type", "Exception")
        lineno = vars.get("lineno", "?")
        
        # Create a defensive try-except wrap
        patch_code = f'''try:
    {original_line.strip()}
except {exception_type} as e:
    # Auto-generated exception handler
    ...
    raise  # Re-raise after logging'''
        ...
        candidates = [
            AFECandidate(
                detection_id=detection_id,
                type="code_patch",
                summary=f"Wrap line {lineno} in try-except {exception_type}",
                diff=f"""# WRAP at line {lineno}:
{patch_code}""",
                confidence=0.6,  # Lower confidence for generic fix
                status="pending",
                validation_results={
                    "patch_type": "wrap",
                    "target_lineno": lineno if isinstance(lineno, int) else 0,
                    "template_based": False,
                    "strategy": "generic_wrap"
                }
            )
        ]
