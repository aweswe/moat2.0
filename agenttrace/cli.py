...
def cmd_replay(args):
    ...
    if args.run:
        ...
        if script_path and os.path.exists(script_path):
            ...
            env = os.environ.copy()
            # Ensure these are set for the subprocess
            env["AGENTTRACE_MODE"] = "REPLAY"
            env["AGENTTRACE_ID"] = trace_id
            if step is not None:
...
def cmd_list(args):
    """List all traces"""
    root = find_trace_root()
    traces_dir = root / ".agenttrace" / "traces"
    ...
    for trace_item in traces_dir.iterdir():
        if not trace_item.is_dir():
            continue
            
        trace_id = trace_item.name
        metadata_file = trace_item / "metadata.json"
                ...
                if events_file.exists():
                     with open(events_file, 'r') as f:
                        ...
                        traces.append({
                            "id": trace_id,
                            "events": count,
                            "start": 0, # Unknown without parsing
                            "end": 0
                        })
        except Exception as e:
            # print(f"Debug: Error reading {trace_id}: {e}")
            pass
...
def main():
    ...
    replay_parser.add_argument("--run", action="store_true", help="Immediately execute the script")
